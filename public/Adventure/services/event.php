	...
	try{
		switch($_SERVER['REQUEST_METHOD']){
			...
			case 'PUT':
				$input = json_decode(file_get_contents('php://input'));
				$validation = new Validation();
				$validation->prime($input, ['ID','adventureID','eventTypeID','flagID','value','textBefore','textAfter','pageID','conditionID','conditionFlagID','counterValue','counterUpperValue']);
				...
				if($validation->result['error'] == 1){
					...
					$event->Update($valid->eventTypeID, $valid->flagID, $valid->value, $valid->textBefore, $valid->textAfter, $valid->pageID, $valid->counterValue, $valid->counterUpperValue, $valid->conditionID, $valid->conditionFlagID);
				}
				break;
			case 'GET':			
				$input = (object)$_GET;
